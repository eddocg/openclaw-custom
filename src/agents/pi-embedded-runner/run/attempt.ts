      ...
      try {
        ...
        try {
          ...
          if (!skipPromptSubmission) {
            ...
            // prompt; the wrap target is the fully layered promptForModel
            // after upstream hook/runtime prompt preparation.
            ...
            const promptForModelWithMemory = await memoryInjector.inject({
              promptToWrap: promptForModel,
              query: params.prompt,
            });
            finalPromptText = promptForModelWithMemory;
            trajectoryRecorder?.recordEvent("prompt.submitted", {
              prompt: promptForModelWithMemory,
              systemPrompt: systemPromptForHook,
              messages: activeSession.messages,
              imagesCount: imageResult.images.length,
            });
            ...
            updateActiveEmbeddedRunSnapshot(params.sessionId, {
              ...
              inFlightPrompt: promptForModelWithMemory,
            });
            if (promptSubmission.runtimeOnly) {
              await promptActiveSession(promptForModelWithMemory);
            } else {
